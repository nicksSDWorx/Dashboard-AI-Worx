#!/usr/bin/env python3
"""
SDWorx 404 Link Checker Agent — v2 (rate-limit fix)
...
Gebruik:
    ...
    python3 sdworx_checker_v2.py --max 10000 --concurrency 4
...
Rate-limit aanpassingen t.o.v. origineel:
    - DELAY      : 0.15s → 1.5s  per worker (10x langer)
    - CONCURRENCY: 8     → 4     workers    (2x minder)
    - Effectieve snelheid: ~53 req/s → ~2.7 req/s
    - Exponential backoff bij 429: wacht 10s, 20s, 40s (max 3 pogingen)
    - Respecteert Retry-After header van de server
"""
...
DOMAIN         = "sdworx.nl"
MAX_PAGES      = 10000
CONCURRENCY    = 4          # ← was 8  (minder gelijktijdige requests)
TIMEOUT        = 12
DELAY          = 1.5        # ← was 0.15s  (10x langzamer per worker)

# Retry-instellingen bij 429
RETRY_MAX      = 3          # max aantal pogingen per URL
RETRY_BACKOFF  = 10.0       # basiswachttijd in seconden (verdubbelt per poging)

EMAIL_TO       = "melanie.brugel@sdworx.com"
EMAIL_FROM     = "noreply-linkchecker@sdworx.com"
SMTP_HOST      = "localhost"
SMTP_PORT      = 587
...
def sync_fetch(url: str) -> tuple:
    ...
    Herprobeert automatisch bij HTTP 429 met exponential backoff.
    Respecteert de Retry-After header van de server.
    """
    for attempt in range(1, RETRY_MAX + 1):
        try:
            ...
            if e.code == 429 and attempt < RETRY_MAX:
                # Lees Retry-After header als die er is, anders exponential backoff
                ...
                if retry_after:
                    ...
                    wait = RETRY_BACKOFF * (2 ** (attempt - 1))  # 10s, 20s, 40s

                print(f"  {C.YELLOW}⏳ 429 ontvangen voor {url[:60]}… "
                      f"wacht {wait:.0f}s (poging {attempt}/{RETRY_MAX}){C.RESET}")
                time.sleep(wait)
                continue
            return e.code, ""
...
def fmt_status(status) -> str:
    s = str(status)
    if status in (404, 410, 429) or status in ("ERR", "TIMEOUT"):
...
class Result:
    ...
    def is_broken(self) -> bool:
        return self.status in (404, 410, 429) or self.status in ("ERR", "TIMEOUT")
...
class LinkCheckerAgent:
    ...
    async def process(self, loop, url: str, link_text: str, found_on: str):
        # Beleefde vertraging per worker voor throttling
    ...
    async def run(self):
        ...
        print(f"\n{C.PURPLE}{C.BOLD}🕷️  SDWorx 404 Link Checker Agent v2 (rate-limit fix){C.RESET}")
        print(f"{C.GRAY}   Seed        : {SEED_URL}")
        print(f"   Max pages   : {self.max_pages}")
        print(f"   Concurrency : {self.concurrency}  (was: 8)")
        print(f"   Delay       : {DELAY}s per worker  (was: 0.15s)")
        print(f"   ~Req/sec    : ~{self.concurrency / DELAY:.1f}")
        print(f"   Retry 429   : max {RETRY_MAX}x, backoff {RETRY_BACKOFF}s/20s/40s")
...
def send_email(
    ...
    subject = (
        f"[SDWorx Link Checker] "
        f"{'🚨 ' + str(broken_count) + ' kapotte links gevonden' if broken_count else '✅ Geen kapotte links'}"
...
Voorbeelden:
  python3 sdworx_checker_v2.py
  python3 sdworx_checker_v2.py --max 500 --concurrency 4
